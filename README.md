...
- 운영 어드민 자동 입력·자동 등록 기능은 구현하지 않습니다.
- 사내 내부 API 호출, 운영 계정 쿠키·토큰 활용, 운영 화면 브라우저 자동화는 범위에서 제외합니다.
- 사람이 최종 판단하고 실행하는 안전한 업무 보조 기능에 집중합니다.
- 우선 개발 영역은 주간 리포트, 탁송 응답, 제주 탁송 문구, 슬랙 보고 문구, 항동센터 가이드, 런처 안정화입니다.
...
- 차량 등급과 현장 상태에 따른 상품화 기준 대조 및 권장 작업 정리
- 현장 약어를 표준 부위명으로 변환
- 추천 작업 및 복붙용 키워드 생성
...
- 콘솔 도구 키워드 검색
- 독립 런처의 프라이빗 배경, 밝기, 블러, PIN 로컬 설정
- 저장 기록 다시 불러오기 및 삭제
- 모바일과 PC 반응형 UI
- 외부 API 없이 브라우저에서 동작
...
- `/remanufacturing/standards`: 상품화 기준 대조
...
독립 `PDI Launcher.exe`의 우측 상단 설정 버튼에서 프라이빗 모드를 활성화하고
로컬 이미지, GIF 또는 MP4 배경을 선택할 수 있습니다.
선택한 파일은 런처의 `launcher-data/private-assets`에 복사됩니다.
